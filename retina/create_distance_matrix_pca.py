...
data = pd.read_pickle(ospath.join(folder_path, "mnist_pca_data_1200_by_784_15.11.58.501123.pkl")) # 3 classes
print(data['Color'])
print(data['Dimension 1'])
...
for im1 in range(data_size):
    for im2 in range(data_size):
        im1_x = data['Dimension 1'][im1]
        im1_y = data['Dimension 2'][im1]
        im2_x = data['Dimension 1'][im2]
        im2_y = data['Dimension 2'][im2]
        distances[im1,im2] = math.sqrt((im1_x-im2_x)**2+(im1_y-im2_y)**2)
        
# with open(ospath.join(folder_path, "distances.npy"), 'wb') as f:
np.save(ospath.join(folder_path, "pca_distances_3.npy"), distances)