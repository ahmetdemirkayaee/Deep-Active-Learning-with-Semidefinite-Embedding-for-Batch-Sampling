...
for e in range(exp):
    for j in range(0,5):
        ...
        # dataset_mnist = np.concatenate(([dataset[abs_labels == k][0:image_per_class,:,:] for k in range(1,number_of_classes+1)]), axis=0)
        
        # dataset_mnist_y = np.concatenate(([abs_labels[abs_labels == k][0:image_per_class,] for k in range(1,number_of_classes+1)]), axis=0)
        # dataset_mnist_y -= 1
        # dataset_mnist_test = np.concatenate(([dataset[y_data == k][-1000:,:,:] for k in range(1,number_of_classes+1)]), axis=0)
        # dataset_mnist_test_y = np.concatenate(([y_data[y_data == k][-1000:,] for k in range(1,number_of_classes+1)]), axis=0)
        # classes = ([str(i) for i in np.unique(dataset_mnist_test_y)])
        # dataset_mnist_test_y -= 1
      ...
      #test_dataset = dataset[]
        ...
        # retina_images_list = retina_images_list[arr]
        # abs_labels = abs_labels[arr]
        
        
          # flattened_data[k] = image.flatten()
        # abs_labels = dataset_mnist_y
        # print(abs_labels)
        # abs_labels[np.where(abs_labels<0)]=0
        # print(np.where(abs_labels==0)[0].shape)
        ...
        # x_data = dataset[:1000,:,:]
        # y_data = abs_labels[:1000]
        
        
        # dataset_mnist = x_data
        # dataset_mnist_y = abs_labels
        # dataset_mnist_test_y = abs_labels[-4000:]
        # dataset_mnist_test = dataset[-4000:,:,:]
        ...
        for cycle in range(active_learning_cycle):
            ...
            # selected_samples = choose_sample_distances(outputs.detach().cpu().numpy(),number_of_classes,'margin sampling',num_of_selection,distances,alpha)
