...
# distances = load_distances_matrix("pca_distances_3.npy")
...
print(dataset_mnist.shape[0])
data_size = dataset_mnist.shape[0]
distances = np.zeros((data_size,data_size))
for im1 in range(data_size):
    for im2 in range(data_size):
        diff_sq = np.square(dataset_mnist[im1,:,:] - dataset_mnist[im2,:,:])
        distances[im1,im2] = np.sum(diff_sq)
        # distances[im1,im2] = distance.euclidean(dataset_mnist[im1,:,:], dataset_mnist[im2,:,:])
        # distances[im1,im2] = math.sqrt((dataset_mnist[im1,:,:]-im2_x)**2+(im1_y-im2_y)**2)
...
for e in range(exp):
    for j in range(0,5):
        ...
        print(test_accs)
        # np.save("results/3class_pca" + str(exp)+"_least_confidence.npy", np.array(test_accs))
        # np.save("results/3class_pca/" + str(seed)+"_least_confidence_distance_"+str(alpha)+".npy", np.array(test_accs))
        # np.save("results/3class_pca/" + str(seed)+"_margin_sampling_distance_"+str(alpha)+".npy", np.array(test_accs))
        # np.save("results/3class_pca/" + str(seed)+"_entropy_distance_"+str(alpha)+".npy", np.array(test_accs))
        # np.save("results/3class_l2/" + str(seed)+"_least_confidence_distance_"+str(alpha)+".npy", np.array(test_accs)) 
        np.save("results/3class_l2/" + str(seed)+"_margin_sampling_distance_"+str(alpha)+".npy", np.array(test_accs)) 