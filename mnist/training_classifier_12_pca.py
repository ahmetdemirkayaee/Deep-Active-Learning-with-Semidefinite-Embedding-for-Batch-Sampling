...
for e in range(exp):
    for j in range(0,5):
        ...
        dataset_mnist = np.concatenate(([dataset[y_data == k][0:image_per_class,:,:] for k in range(1,number_of_classes+1)]), axis=0)
        
        dataset_mnist_y = np.concatenate(([y_data[y_data == k][0:image_per_class,] for k in range(1,number_of_classes+1)]), axis=0)
        dataset_mnist_y -= 1
        ...
        np.save("results/2class_pca/" + str(seed)+"_least_confidence_distance_"+str(alpha)+".npy", np.array(test_accs))
        # np.save("results/" + str(seed)+"_entropy_distance_"+str(alpha)+".npy", np.array(test_accs))
        # np.save("results/" + str(seed)+"_entropy_distance_"+str(alpha)+".npy", np.array(test_accs))
